...
def checkInput(userInput, opt):
    ...
    while (userInput.isdigit() == False) or (int(userInput) > gameBoard.size) or (int(userInput) < 1):
        if not userInput.isdigit():
            ...
            userInput = input("Pick a %s [1~%d]: " % (opt, gameBoard.size))
        else:
            print("Oops, that's not even in the ocean. Pick a row between 1 and %d" % gameBoard.size)
            userInput = input("Pick a %s [1~%d]: " % (opt, gameBoard.size))
    ...
    for ship in shipsList:
        if mark == ship.mark:
            ...
            if ship.getLife == 0:
                ...
                print("*** Hooyah! %s was destroyed ***" % ship.name)
# end
...
def startGame():
    ...
    for turn in range(0, num_tries):
        ...
        rowPicked = input("Pick a row [1~%d]: " % gameBoard.size)
        rowPicked = checkInput(rowPicked, "row")
        colPicked = input("Pick a col [1~%d]: " % gameBoard.size)
